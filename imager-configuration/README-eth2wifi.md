# Raspberry Pi Image Configuration for Ethernet to WiFi connection
Step-by-step instructions for configuring Raspberry Pi Image for use with the **Ethernet to WiFi** [VPNTV](https://github.com/d3vilh/vpntv) connection option.
...
5. Click on the `"GEARBOX"` (Step3) icon to open the "Advanced Options" window and configure the following options:
   ...
   2. Set the uniq password for the `vpntv` user. For example, `Uniqu3_pa$Sword!:)` and skip the SSID configuration (buildin WiFi module will be used as AP):
...
8. Plug your Raspberry Pi to the power supply and **Ethernet port of your router**. Wait for the first boot, it can take up to the 5 minutes, depends on board you use.
